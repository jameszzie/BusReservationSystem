    ...
    while (1) {
        ...
        switch(choice) {
            ...
            case 3: 
                ...
                // 1. Check if the bus exists and seat is already booked
                ...
                if(busIdx != -1) {
                    if(sn >= 1 && sn <= MAX_SEATS) {
                        if(buses[busIdx].seats[sn-1] == 1) {
                            printf("[Error] Seat %d on Bus %d is already booked!\n", sn, id);
                            isDuplicate = 1;
                        }
                    } else {
                        printf("[Error] Invalid seat number. Please choose 1-%d.\n", MAX_SEATS);
                        isDuplicate = 1;
                    }
                } else {
                    printf("[Error] Bus ID %d does not exist.\n", id);
                    isDuplicate = 1;
                }
                
                // 2. Check if the request is already in the Queue
                if (!isDuplicate) {
                    for(i = front; i < rear; i++) { 
                        if(queue[i].bus_id == id && queue[i].seat_num == sn) {
                            printf("[Error] This request is already waiting in the queue.\n");
                            isDuplicate = 1;
                            break;
                        }
                    }
                ...
                if(!isDuplicate) {
                    enqueue(id, sn);
                    printf("[Success] Added Bus %d Seat %d to the booking queue.\n", id, sn);
                }
                break;
